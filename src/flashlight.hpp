...
inline glm::mat4 viewProjectionMatrix(float aspect = 1.0f, float /*time*/ = 0.0f) {
  return projectionMatrix(aspect) * viewMatrix();
}

// biased transform for shadow sampling in the lit pass
inline glm::mat4 biasedLightSpaceMatrix(float aspect = 1.0f, float /*time*/ = 0.0f) {
...
inline bool hitsPoint(const glm::vec3 &p, float /*time*/ = 0.0f) {
  if (!enabled) {
    return false;
  }
  const glm::vec3 toP = p - position();
  const float dist = glm::length(toP);
  if (dist > kRange || dist < 0.001f) {
    return false;
  }
